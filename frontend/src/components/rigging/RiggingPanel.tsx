 ...
 *
 * Supports two modes:
 * - Auto-Rig: Automatic skeleton generation and fitting
 * - Landmarks: User positions 8 key landmarks, system generates skeleton
 */
...
type Step = 'config' | 'processing' | 'complete' | 'error' | 'landmarks';
type RigMode = 'auto' | 'landmarks';
...
export function RiggingPanel({ assetId, onClose, className = '' }: RiggingPanelProps) {
  ...
  const [rigMode, setRigMode] = useState<RigMode>('auto');
  ...
  return (
    <div className={`flex flex-col h-full bg-surface ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Mode Selector - shown in config step */}
        {step === 'config' && (
          <div className="flex rounded-lg bg-surface-light border border-border p-1">
            <button
              onClick={() => setRigMode('auto')}
        ...
        {step === 'config' && rigMode === 'landmarks' && (
          <>
            {/* Asset Preview for Landmark Mode */}
            {targetAsset ? (
              <div className="p-3 bg-surface-light rounded-lg border border-border">
                <div className="flex items-center gap-3">
                  {targetAsset.thumbnailPath ? (
                  ...
                  <div>
                    <p className="font-medium text-text-primary">{targetAsset.name}</p>
                    <p className="text-sm text-purple-400">Landmark Mode</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="p-6 text-center text-text-muted">
                <p>Select an asset to rig with landmarks</p>
              </div>
            )}
